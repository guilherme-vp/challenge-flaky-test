describe('Sign Up', () => {
	it('Adds person to course', () => {
		...
		cy.get('h1').should('contain', 'Sign Up Sheet')
		...
		cy.get('select[name="course"]').should('not.exist')

		cy.get('h3').should('contain', 'People')

		cy.get('img[alt="loading"]').should('exist').and('have.attr', 'src', '/img/loading.gif')
	...
	it('Displays error and disable submit button if email is not given or invalid', () => {
		cy.visit('/')
		...
		cy.get('input[name="email"]').type('some').should('have.value', 'some')

		cy.get('span').eq(1).should('contain', 'Invalid Email')
		...
		cy.get('span').eq(1).should('contain', '')
